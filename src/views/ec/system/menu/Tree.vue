<template>
  <div class="bg-white m-4 mr-2 overflow-hidden">
    ...
    <BasicTree
      :title="t('ec.system.menu.table.title')"
      toolbar
      checkable
      search
      checkStrictly
...
<script lang="ts">
  ...
  import { tree, remove } from '/@/api/ec/system/menu';
    ...
    setup(_, { emit }) {
      ...
      function handleSelect(keys: string[]) {
        if (keys[0]) {
          ...
          emit('select', parent, node, treeData.value);
        }
      }
          ...
          render: (node) => {
            return h(PlusOutlined, {
              ...
              onClick: (e) => {
                e?.stopPropagation();
                e?.preventDefault();
                emit('add', findNodeByKey(node.id, treeData.value), treeData.value);
              },
            });
          },
        },
        ...
        return [
          {
            ...
            handler: () => {
              emit('add', findNodeByKey(unref(node).id, treeData.value), treeData.value);
            },
            icon: 'bi:plus',
          },
          {
      ...
      // 点击组织数外面的 新增
      function handleAdd() {
        emit('add', findNodeByKey('0', treeData.value), treeData.value);
      }
