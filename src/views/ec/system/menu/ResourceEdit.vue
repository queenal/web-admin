...
<script lang="ts">
  ...
  import { Api, save, update } from '/@/api/ec/system/resource';
  import { getValidateRules } from '/@/api/ec/common/formValidateService';
  import { resourceCustomFormSchemaRules, resourceEditFormSchema } from './menu.data';
  ...
  export default defineComponent({
    ...
    setup(_, { emit }) {
      ...
      const [registerForm, { setFieldsValue, resetFields, updateSchema, validate }] = useForm({
        labelWidth: 100,
        schemas: resourceEditFormSchema,
      ...
      const [registerDrawer, { setDrawerProps, closeDrawer }] = useDrawerInner(async (data) => {
        ...
        record.menuName = data?.menuName || '';
        await setFieldsValue({ ...record });

        getValidateRules(validateApi, resourceCustomFormSchemaRules(type)).then(async (rules) => {
